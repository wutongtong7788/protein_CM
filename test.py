...
def calculateEvaluationStats(_pred_cmap, _true_cmap, L, _name):
    ...
    # pred_cmap = _pred_cmap.cpu()
    # true_cmap = _true_cmap.cpu()
    ...
    if  max_Top ==50:
        print(_name)##########看有没有比50小的

    for i in range(1, max_Top + 1):
        (x, y) = np.unravel_index(np.argmax(pred_cmap, axis=None), pred_cmap.shape)
        # np.argmax(pred_cmap)是把数组拉直最大数的索引，np.unravel_index是返回最大数在原数组的索引，（80，102）这个位置
        pred_cmap[x][y] = 0
        if true_cmap[x][y] == 1:
            con_num += 1
        if i == 5:
            prec_T5 = con_num * 20  #因为f分母是5，5*20到100,所以分子*20
...
def get_evaluation_result(_arr, _relax, _SAMPLE_SIZE):
    ...
    return some_array,str(sum_prec_T10 / _SAMPLE_SIZE),str(sum_prec_L30 / _SAMPLE_SIZE)
...
# SAMPLE_SIZE = 0
#
# # real_cmap = '../example/3GWR/3GWR.cmap'
# # predict_cmap = '../3GWR/cmap_3GWR.cmap'
# # pre='D:/C_Res2Net-PretrainedModels-master/Res2Net-PretrainedModels-master/pre_3_23_49/'
# # pre='E:/u2020103588_139_214/139_3_27_2/test_27/pre_3_27/'
# pre='E:/u2020103588_139_214/214_4_13_5/pre_4_13_5/'
# # label='D:/C_Res2Net-PretrainedModels-master/Res2Net-PretrainedModels-master/label_3_23_49/'
# # label='E:/u2020103588_139_214/139_3_27_2/test_27/label_3_27/'
# label='E:/u2020103588_139_214/214_4_13_5/label_4_13_5/'
# dir=os.listdir(pre)
# for ii in dir:
#     real_cmap =label+ ii
#     # print(real_cmap)
